...
| 시간대 | `Asia/Seoul` 명시 |
| 테스트 | `pytest`; 순수 로직 단위 테스트와 향후 외부 클라이언트 대역 테스트 |
...
Google Cloud 프로젝트에는 Sheets API와 초기 파일 생성·공유에 사용한 Drive API가 활성화되어 있다. 일일 애플리케이션은 Drive API를 호출하지 않는다. 드라이런은 Sheets 읽기 전용 인증 범위를 사용하고, 향후 게시 작업은 검증과 중복 확인 후 추천 로그를 추가할 수 있는 Sheets 범위를 사용한다.
...
- 초기 실행 골격의 service는 외부 연동을 수행하지 않으므로 일일 작업 완성 전에는 활성화하지 않는다.
