...
2026-07-21 후보 웹의 첫 실제 등록에서는 `사용=FALSE`인 빈 체크박스 행이 1000행까지 미리 채워져 있어 Sheets API의 append가 새 후보를 1001행에 기록했다. 등록 방식은 식당과 메뉴가 모두 비어 있는 첫 행의 `A:G`를 갱신하도록 변경했다. 이미 입력된 후보나 식당·메뉴 중 하나만 작성된 행은 덮어쓰지 않는다. 1001행의 후보는 값과 형식을 확인한 뒤 8행으로 옮기고 원본 값을 비운 후 API 재조회로 검증했다.
