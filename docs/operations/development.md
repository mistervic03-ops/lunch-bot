...
- 구현됨: 환경변수 검증, 순수 추천 계산, Slack 메시지 생성·게시, 번호 반응 추가·조회, 최근 다섯 메시지 좋아요 동기화, Google Sheets 후보와 추천 로그 읽기·검증 및 일괄 쓰기, 안전한 읽기·동기화의 제한된 재시도, 날짜·채널 중복 판정, 운영 채널 알림, 실제 일일 작업, 읽기 전용 드라이런, Slack 연결 테스트 명령, systemd 실패 알림 명령, 최소 권한 Slack App Manifest, systemd 템플릿
...
```text
bapratustra/
├── __main__.py        # 설정 검증, 드라이런, 연결 테스트와 일일 작업 진입점
...
├── messaging.py       # Slack 추천 메시지, 번호 반응과 운영 알림
...
첫 명령은 단위 테스트를 실행한다. 두 번째 명령은 전체 필수 설정과 인증 파일 경로만 검사한다. 세 번째 명령은 실제 후보와 로그를 읽고 추천 메시지를 터미널에 출력하지만 Sheet를 수정하거나 Slack에 게시하지 않는다. 네 번째 명령은 표시된 연결 테스트 메시지와 번호 반응을 실제 테스트 채널에 한 번 게시·조회하지만 추천 로그를 수정하지 않는다. 마지막 명령은 최근 좋아요를 갱신하고, 중복 여부를 확인한 뒤 실제 추천 게시·로그 기록·번호 반응 추가를 수행한다.
