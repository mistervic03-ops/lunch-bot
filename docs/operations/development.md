...
- `python -m babgwe --run-daily`는 실제 Slack과 Google Sheet를 변경한다.
- 실제 점심·운영 채널에서 전체 흐름을 한 번 검증하기 전에는 `deploy/babgwe.timer`를 활성화하지 않는다.
...
- service는 `python -m babgwe --run-daily`를 실행한다.
