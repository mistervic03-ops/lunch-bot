...
- 구현됨: 환경변수 검증, 순수 추천 계산, Slack 메시지와 Sheet·전당 링크 버튼 생성·게시, 채널 온보딩 게시 명령, 링크 버튼 Socket Mode ACK, 번호 반응 추가·조회, 최근 다섯 메시지 좋아요 동기화, Google Sheets 후보와 추천 로그 읽기·검증 및 일괄 쓰기, 안전한 읽기·동기화의 제한된 재시도, 날짜·채널 중복 판정, 운영 채널 알림, 실제 일일 작업, 읽기 전용 드라이런, Slack 연결 테스트 명령, systemd 실패 알림 명령, 사내 리더보드 집계·캐시·HTML과 health check, 격리된 SQLite 후보 관리 알파와 백업·Sheet 가져오기, Slack App Manifest, systemd 템플릿
...
├── database.py        # 알파 후보·로그·변경 이력 SQLite와 백업
├── alpha_import.py    # Sheet를 수정하지 않는 일회성 알파 가져오기
├── alpha_web.py       # 별도 후보 추가·검색·수정·비활성화 웹 화면
...
후보 관리 알파는 `BAPRATUSTRA_ALPHA_DB`와 `BAPRATUSTRA_ALPHA_BACKUP_DIR`만으로 별도 포트에서 실행한다. Sheet 가져오기 명령은 Google 설정도 읽지만 읽기 전용 API만 사용하며 비어 있지 않은 DB를 덮어쓰지 않는다. `--backup-alpha`는 무결성 검사를 통과한 새 백업을 만든 뒤 이름 기준 최신 30개만 남긴다. 웹 수정마다 백업하거나 화면에서 변경을 되돌리는 기능은 두지 않는다. 현재 운영 봇과 전당은 알파 DB를 참조하지 않는다.
...
- `bapratustra-alpha.service`는 별도 SQLite 경로를 사용하는 Uvicorn 단일 worker를 `0.0.0.0:8031`에서 실행하고 비정상 종료 때 재시작한다.
- `bapratustra-alpha-backup.timer`는 매일 03:30 KST에 `bapratustra-alpha-backup.service`를 실행한다. 놓친 실행은 다음 기동 때 한 번 보충하며 백업 실패는 기존 systemd 실패 알림으로 운영 채널에 전달한다.
- 알파 DB와 백업 디렉터리는 `bapratustra` Unix 사용자가 읽고 쓸 수 있어야 한다. 원본과 백업이 같은 디스크에 있으므로 원본 전환 전에는 서버 밖 복제 위치를 별도로 확정한다.
