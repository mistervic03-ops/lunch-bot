...
- 구현됨: 환경변수 검증, 순수 추천 계산, Slack 메시지 생성·게시, 번호 반응 추가·조회, 최근 다섯 메시지 좋아요 동기화, Google Sheets 후보와 추천 로그 읽기·검증 및 일괄 쓰기, 날짜·채널 중복 판정, 운영 채널 알림, 실제 일일 작업, 읽기 전용 드라이런, Slack 연결 테스트 명령, 최소 권한 Slack App Manifest, systemd 템플릿
