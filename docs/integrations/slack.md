...
밥괘는 워크스페이스 내부에서만 사용하는 일반 Slack 앱이다. 앱 설정의 재현 가능한 원본은 `slack/app-manifest.json`이다. Manifest에는 비밀정보나 워크스페이스별 채널 ID를 넣지 않는다.
...
Socket Mode, 이벤트 구독, Slash Command, Interactivity와 Incoming Webhook은 사용하지 않는다.
...
4. Slack이 보여주는 앱 이름과 세 가지 Bot Token Scope를 확인한 뒤 앱을 생성한다.
...
2. 두 채널에서 `/invite @밥괘`로 봇을 직접 초대한다.
...
- 실제 후보와 추천 로그를 읽어 `[밥괘 연결 테스트]`가 붙은 메시지를 테스트 채널에 한 건 게시한다.
...
- `.env`는 Git에서 제외한다.
- Bot Token은 로그나 오류 메시지에 포함하지 않는다.
- DGX Spark에서는 `/etc/babgwe/babgwe.env`처럼 애플리케이션 저장소 밖의 제한된 환경 파일에 둔다.
- 토큰이 노출되면 Slack 앱 설정에서 즉시 재발급 또는 재설치하고 서버 값을 교체한다.
...
- 테스트 채널 ID, 비공개 운영 채널 ID와 Bot User OAuth Token은 로컬 `.env`에만 둔다.
