...
- 정기 메시지 게시
- 정기 메시지와 온보딩의 Google Sheet·전당 링크 버튼
- 온보딩 메시지 자동 고정
- 후보별 번호 반응 추가
- 기존 번호 반응 수 조회
...
- 저장소 Manifest는 Sheet와 전당 링크 버튼을 위해 Socket Mode와 Interactivity가 활성화된 상태다.
...
- 같은 날 `pins:write`를 추가한 Manifest를 적용하고 앱을 재설치했다. 테스트 채널에서 `--post-onboarding`으로 메시지 `1784533165.924839`를 게시한 직후 자동 고정이 성공해 권한과 워크스페이스 정책을 함께 검증했다.

## 권한과 토큰

Bot Token Scope는 다음 네 개만 사용한다.

| 권한 | 사용 목적 |
|---|---|
| `chat:write` | 봇이 참여한 점심 채널과 운영 채널에 메시지를 게시한다. |
| `pins:write` | 채널 온보딩 메시지를 게시 직후 자동 고정한다. |
...
4. Slack이 보여주는 앱 이름, 네 가지 Bot Token Scope, Socket Mode와 Interactivity 활성화를 확인한 뒤 앱을 생성한다.
...
3. 명령이 메시지 게시와 자동 고정을 모두 완료했는지 확인한다.
4. 필요하면 채널 북마크에 Google Spreadsheet를 `점심 후보`, 웹 리더보드를 `밥라투스트라의 전당`이라는 이름으로 각각 추가한다.
...
- 일반 정기 게시와 같은 `점심 후보 보태기`, `전당 둘러보기` 링크 버튼을 포함한다.
